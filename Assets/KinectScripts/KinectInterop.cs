...
//		}
	...
	public static Vector2 GetKinectPointColorCoords(SensorData sensorData, Vector3 kinectPos)
	{
		Vector2 vPoint = Vector2.zero;
		...
		if(sensorData.coordMapper != null)
		{
			...
			sensorData.coordMapper.MapCameraPointsToColorSpace(camPoints,colorPoints);
			
			ColorSpacePoint colorPoint = colorPoints[0];
			
			if(colorPoint.X >= 0 && colorPoint.X < sensorData.colorImageWidth &&
			   colorPoint.Y >= 0 && colorPoint.Y < sensorData.colorImageHeight)
			{
				vPoint.x = colorPoint.X;
				vPoint.y = colorPoint.Y;
			}
		}
